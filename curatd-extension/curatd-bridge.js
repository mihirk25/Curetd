...
(function () {
  ...
  async function syncSession() {
    ...
    // IndexedDB can briefly return empty during page load; require consecutive
    // empty reads before telling the background page to clear a valid session.
    if (!session) {
      emptySessionReads += 1;
      if (emptySessionReads < 3) return;
    } else {
      emptySessionReads = 0;
    }
